...
class JobVectorStore:
    ...
    def __init__(self, model_name: str = 'all-mpnet-base-v2'):
        ...
        self.index = None
        self.jobs_metadata = []  # Metadata des offres (title, company, description, etc.)
        
        print(f"✅ JobVectorStore initialisé avec {model_name} ({self.dimension} dimensions)")
    ...
    def build_index(self, jobs: List[Dict], index_type: str = 'flat') -> None:
        ...
        if index_type == 'flat':
            ...
            self.index = faiss.IndexFlatIP(self.dimension)  # Inner Product (cosine avec normalisation)
        elif index_type == 'ivf':
    ...
    def search(
        ...
        if self.index is None:
            raise ValueError("L'index FAISS n'est pas construit. Appelez build_index() d'abord.")
        ...
        top_k = min(top_k, self.index.ntotal)  # Limiter au nombre d'offres disponibles
        ...
        for i, idx in enumerate(indices[0]):
            ...
            similarity_score = float(scores[0][i])  # Score de similarité (0-1 avec normalisation)
    ...
    def save(self, index_path: str, metadata_path: str) -> None:
        ...
        """
        if self.index is None:
            raise ValueError("Aucun index à sauvegarder. Appelez build_index() d'abord.")
